...
try {
    ...
    $ws.Name = "Query Configuration Test Cases"

    # Headers
    $headers = @("Test Case ID", "Module", "Test Case Description", "Steps", "Expected Result", "Actual Result", "Comments")
    for ($c=1; $c -le 7; $c++) { 
    ...
    foreach ($case in $cases) {
        for ($col = 1; $col -le 7; $col++) {
            $ws.Cells.Item($row, $col).Value2 = $case[$col-1]
        }
        $row++
    }
    ...
    Write-Host "Excel file created at: $filePath"
} finally {
