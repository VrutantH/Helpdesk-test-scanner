...
$cases = @(
    ...
    # PHASE 5: FORM FIELDS (EXTENDED VERIFICATION)
    ...
    @("QC_47", $M, "Verify that clicking 'Save' persists the form fields to the project.", "1. Click 'Save All Changes'.", "Success message is displayed.", "", ""),
    ...
    @("QC_55", $M, "Verify that clicking 'Save' persists the table column configuration.", "1. Click 'Save All Changes'.", "Success message is displayed.", "", ""),
    ...
    @("QC_57", $M, "Verify that the 'Save All Changes' button is enabled only when changes exist.", "1. Observe button state without changes.`n2. Make a change.`n3. Observe again.", "Button is disabled initially; becomes enabled (purple) after any change.", "", ""),
    @("QC_58", $M, "Verify that clicking 'Save All Changes' displays a global success notification.", "1. Click the main 'Save All Changes' button.", "A toast/alert confirming all settings are updated appears.", "", ""),
    @("QC_59", $M, "Verify that all changes across all 5 tabs are reflected after a full logout/login.", "1. Log out.`n2. Log in.`n3. Check all tabs in 'Auto12'.", "All configurations across Numbering, Statuses, Categories, Fields, and Columns are persisted.", "", ""),
    @("QC_60", $M, "Verify that changes made to project 'Auto12' do not affect other projects.", "1. Open configuration for a different project (e.g., 'Demo').", "The configuration settings for 'Demo' remain in their original state.", "", "")
)
...
Write-Host "Full CSV file created at: $csvPath"
